...
void USlimerAbilitySystemComponent::AddCharacterAbilities(const TArray<TSubclassOf<UGameplayAbility>>& StartupAbilities)
{
	for (TSubclassOf<UGameplayAbility> AbilityClass : StartupAbilities)
	{
		FGameplayAbilitySpec AbilitySpec = FGameplayAbilitySpec(AbilityClass, 1);

		if (const UTowerDefenseGameplayAbility* SlimerAbility = Cast<UTowerDefenseGameplayAbility>(AbilitySpec.Ability))
		{
			AbilitySpec.DynamicAbilityTags.AddTag(SlimerAbility->StartupInputTag);
			GiveAbility(AbilitySpec);
		}
	}
}

void USlimerAbilitySystemComponent::AbilityInputTagHeld(const FGameplayTag& InputTag)
{
